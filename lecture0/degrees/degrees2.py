...
def shortest_path(source, target):
    ...
    # Lets make a source node,frotier and as for taget node , we will jsut check if node.satate = target(it id an id)
    # Node self.state = person_id and self.action is movie_id , ie the movies they starred in togeather.
    ...
    while not frontier.empty():
        ...
        # check if the current node is target
        if current.state == target:
            # if target start backtracking
            path = []
